...
type loginAccountType = (arg1:string,arg2:string) => Promise<{data:{token:string}}|null>;
...
export const getFolders=async(api_key:string)=>{
    return (await axios.get(baseUrl+'/users/folders',getConfig(api_key))).data as Array<folder>
}

export const createFolder=async(api_key:string,foldername:string)=>{
    return await axios.post(baseUrl+'/users/newfolder',{foldername},getConfig(api_key)) as Array<folder>;
}
...
export const getAllCards = async(token:string) => {
    const response:{data:Array<character>}=await (axios.get(baseUrl+'/cards',getConfig(token)));
